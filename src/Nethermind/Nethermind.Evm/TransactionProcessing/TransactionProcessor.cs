...
namespace Nethermind.Evm.TransactionProcessing
{
    public class TransactionProcessor : ITransactionProcessor
    {
            ...
            ExecutionOptions executionOptions)
        {
            ...
            if (notSystemTransaction)
            {
                _logger.Info($"TransactionProcessor not system tx");
                if (gasLimit < intrinsicGas)
                {
                    _logger.Info($"TransactionProcessor not system tx quick fail 1");
                ...
                {
                    _logger.Info($"TransactionProcessor not system tx quick fail 2");
            ...
            _logger.Info($"TransactionProcessor after not system tx");


            if (!_stateProvider.AccountExists(caller))
            {
                _logger.Info($"TransactionProcessor in if !AccountExists");
            ...
            _logger.Info($"TransactionProcessor after if !AccountExists");
            ...
            if (notSystemTransaction)
            {
                _logger.Info($"TransactionProcessor in if notSystemTransaction");
            ...
            _logger.Info($"TransactionProcessor after if notSystemTransaction");


            _stateProvider.SubtractFromBalance(caller, senderReservedGasPayment, spec);
            _logger.Info($"TransactionProcessor after _stateProvider.SubtractFromBalance");

            if (commit)
            {
                _logger.Info($"TransactionProcessor in if commit");

                _stateProvider.Commit(spec, txTracer.IsTracingState ? txTracer : NullTxTracer.Instance);
            }

            _logger.Info($"TransactionProcessor after if commit");
